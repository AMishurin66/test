...
int main(void)
{
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  ...
	  i<<2 > 999 ? sprintf((char *)&text,"Bri: %d",(i+=10)<<2) :
	  			  i<<2 > 99 ? sprintf((char *)&text,"Bri: %d ",(i+=10)<<2) :
	  					  i<<2 > 9 ? sprintf((char *)&text,"Bri: %d  ",(i+=10)<<2) :
	  							sprintf((char *)&text,"Bri: %d   ",(i+=10)<<2);

	  LCD_ShowString(0, 0, 160, 16, text);
	  //LCD_SetBrightness(i<<2);
